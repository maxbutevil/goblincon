...
export function state(): { state: State<Page>, cleanup: () => void } {
	
	const state = new State<Page>(Enum.unit("start"));
	const cleanup = Signal.group(
		
		client.use(INC, OUT),
		INC.subscribe("waiting", (kind) => {
			switch(kind) {
				case "start": state.set(Enum.unit("start")); break;
				case "results": case "score": state.set(Enum.unit("score")); break;
				case "draw": state.set(Enum.unit("drawingSubmitted")); break;
				case "vote": state.set(Enum.unit("voteSubmitted")); break;
			}
		}),
		...
		INC.subscribe("voting", ({ choices, secsLeft }) => {
			...
			state.set(Enum.variant("vote", { choices, endTime }));
		}),
		
		drawingSubmitted.subscribe(() => state.set(Enum.unit("drawingSubmitted"))),
		voteSubmitted.subscribe(() => state.set(Enum.unit("voteSubmitted"))),
	);
	
	return { state, cleanup };
	
}
export function Component({ state }: { state: State<Page> }) {
	
	//Utils.useExternal(page);
	Utils.useExternal(state);
	
	let page = state.get();
	switch(page.key) {
		case "draw": return <Draw goblinName={page.goblinName} endTime={page.endTime} />;
		case "drawingSubmitted": return <DrawingSubmitted />;
		case "vote": return <Vote choices={page.choices} endTime={page.endTime} />;
	...
	/*const [tab, setTab] = React.useState<keyof typeof Tabs>("Start");//"Draw");//"Landing");
	const Tab = Tabs[tab];
	
	React.useLayoutEffect(() => Signal.group(
		drawingSubmitted.subscribe(() => { setTab("DrawingSubmitted") }),
		voteSubmitted.subscribe(() => { setTab("VoteSubmitted") }),
		INC.subscribe("waiting", (kind) => {
			switch(kind) {
		...
		INC.subscribe("drawing", ({ goblinName, secsLeft }) => {
			drawGoblinName = goblinName as string;
			drawEndTime = Date.now() + 1000 * (secsLeft - 1);
			setTab("Draw");
		}),
		
		//client.inc.subscribe("scoring", () => setTab("Score")),
		INC.subscribe("voting", ({ choices, secsLeft }) => {
			voteEndTime = Date.now() + 1000 * (secsLeft - 1);
			voteChoices = choices;
			setTab("Vote");
		}),
		client.use(INC, OUT),
