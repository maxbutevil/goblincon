...
function Landing() {
	...
	function joinGame(code: string, name: string) {
		...
		client.connect(`${Utils.wsRoot}/play/join?code=${Globals.joinCode}&name=${name}`);
	}
	...
	return (
		<div className="tab" id="landing">
			...
			{canHost && <a href="/host" id="footer">Hosting a game? Click here</a>}
		</div>
	);
}
...
function App() {
	...
	React.useEffect(() => {
		...
		if (rejoinInfo) {
			...
			client.connect(`${Utils.wsRoot}/play/rejoin?${params}`);
		}
	}, []);
	...
	React.useEffect(() => Signal.group(
		...
		page.changed.subscribe(([from, _to]) => {
			if ("cleanup" in from) from.cleanup();
			/*switch(from.key) {
